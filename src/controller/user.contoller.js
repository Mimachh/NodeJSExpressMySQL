...
export const getUsers = (req, res) => {
    ...
    database.query(QUERY.SELECT_USERS, (error, results) => {
        if(!results) {
            res.status(HttpStatus.OK.code).send(new Response(HttpStatus.OK.code, HttpStatus.OK.status, `No user found`));
        } else {
            res.status(HttpStatus.OK.code).send(new Response(HttpStatus.OK.code, HttpStatus.OK.status, `User retrieved`, { users: results}));
        }
    });
};
...
export const createUser = (req, res) => {
    ...
    database.query(QUERY.CREATE_USER_PROCEDURE, Object.values(req.body), (error, results) => {
        if(!results) {
            ...
            // version sans procédure : const user = { id: results.insertId, ...req.body, created_at: new Date() };
